...
public class KeyBoardToolBoxView extends RelativeLayout {
     ...
     * 发送表情开关按钮
     */
    @BindView(R2.id.keyboard_emotion_button)
    CheckBox mButtonEmotion;
    /**
     * 发送更多类型消息开关按钮
     */
    @BindView(R2.id.keyboard_moreFun_button)
    CheckBox mButtonMoreFun;
    /**
     * 切换到语音输入按钮
     */
    @BindView(R2.id.keyboard_voice_button)
    CheckBox mButtonVoice;
    /**
     * 发送文本消息按钮
     */
    @BindView(R2.id.keyboard_sendText_button)
    Button mButtonSendText;
    /**
     * 文本消息输入框
     */
    @BindView(R2.id.keyboard_input_editText)
    EditText mInputEditText;
    /**
     * 语音录音按钮
     */
    @BindView(R2.id.keyboard_record_voice_button)
    ...
    private void initView(Context context) {
        ...
        mInputEditText.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (TextUtils.isEmpty(text)) {//切换语音输入与文本输入
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (ActivityCompat.checkSelfPermission(getContext(), Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) {
                        ToastyUtil.infoShort( "此应用录音权限已被禁止，请先打开");
                    } else {
