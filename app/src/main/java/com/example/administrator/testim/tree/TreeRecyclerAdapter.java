package com.example.administrator.testim.tree;
...
//        convertView = getConvertView(node, position, convertView, parent);
        // 设置内边距
        holder.itemView.setPadding((int) (node.getLevel() * (PDpAndPx.dip2px(Utils.getApp(), 18))), 3, 3, 3);
        /**
        ...
        holder.itemView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if (onTreeNodeClickListener != null) {
                    onTreeNodeClickListener.OnLongClick(mNodes.get(position),
                            position);
                }
                return true;
            }
    ...
    public void addData(int index, List<Node> mlists, Node node) {
        notifyData(index, mlists, node);
    }
     ...
     * @param mListNodes
     */
    private void notifyData(int index, List<Node> mListNodes, Node fNode) {
        ...
        for (int i = 0; i < mAllNodes.size(); i++) {
            ...
            node.getChildren().clear();
            node.isNewAdd = false;
            if (node.getId().equals(fNode.getId())) {
                mAllNodes.get(i).setAdd(false);
                if (!mAllNodes.get(i).isExpand())
                    mAllNodes.get(i).setExpand(true);
            }
        }
        if (index != -1) {
