...
import com.example.administrator.testim.presenter.LoginPresent;
...
public class LoginActivity extends BaseActivity<LoginPresent> {
    ...
    EditText edtUsrName;
    @BindView(R.id.edt_UsrPwd)
    EditText edtUsrPwd;
    @BindView(R.id.btn_foeget_pw)
...
//                FileUtils2.init(UserManager.getAppUserModel().getUserName());
    ...
    @OnClick({R.id.btn_Login, R.id.btn_foeget_pw})
...
//                            NetRequestCode.NET_REQUEST_1);
    ...
    public void login() {
        ...
        Observable<String> observable = SmackApplication.superApi.login(edtUsrName.getText().toString().trim(),
                "APP", jsonObject);
